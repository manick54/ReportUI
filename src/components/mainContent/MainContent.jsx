...
import {getFilterListAsync} from '../reportSlice';
import {filterListJson} from '../reportSlice';
...
export default function MainContent(props) {
  ...
  const filterList = useSelector(filterListJson);

  const bull = <span className={classes.bullet}>•</span>;

  
  useEffect(() => {
   dispatch(getFilterListAsync());
  ...
  return (
      ...
      <List>
        {filterList.map((text, index) => (
          ...
          <List component="div" >
            ...
            {text.type=="list" && text.subMenu.map(x => {
              return(
    ...
    {reportList.slice(firstIndex,lastIndex).map((text, index) => (
      ...
      <CardContent>
      <Grid container alignItems="center" className={classes.root1}>
